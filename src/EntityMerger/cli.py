"""
CLI for running the EntityMerger process.
"""
...
from EntityMerger.EntityMerger import EntityMerger
from Utils.logging import setup_logging

# Create a Typer app instance
app = typer.Typer(
    name="entity-merger",
    help="A CLI to merge entity suggestions from different audit phases.",
...
def run(
    ...
    """Runs the entity merging workflow for a specific tenant and month."""
    # 1. Setup Logging
    try:
    ...
    logger.info(f"Starting entity merging for Tenant: {tenant}, Period: {year}-{month}")
    ...
    try:
        ...
        merger.merge_suggestions()
        
        logger.info("Entity merging completed successfully!")
        
    except Exception as e:
        logger.error(f"An error occurred during the entity merging workflow: {e}", exc_info=True)
